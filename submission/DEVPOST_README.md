...
<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/demo-cards/banner-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/demo-cards/banner-light.svg">
    <!-- ⬇ banner size — change this one width -->
    <img width="620" src="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/demo-cards/banner-light.svg"
...
[![Jupyter](https://img.shields.io/badge/Demo-Jupyter%20Notebook-F37626?logo=jupyter&logoColor=white)](https://github.com/iarjunganesh/continuum/blob/main/notebooks/DEMO_RUNBOOK.ipynb)
...
<p align="center">
  <a href="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/architecture-diagram-light.svg" target="_blank" rel="noopener noreferrer">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/architecture-diagram-dark.svg">
      <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/architecture-diagram-light.svg">
      <img width="900" src="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/architecture-diagram-light.svg"
...
<sub>Click to enlarge (opens the full-resolution SVG — scales without pixelation): <a href="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/architecture-diagram-light.svg" target="_blank" rel="noopener noreferrer">light</a> / <a href="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/architecture-diagram-dark.svg" target="_blank" rel="noopener noreferrer">dark</a> · Source: <a href="https://github.com/iarjunganesh/continuum/blob/main/assets/architecture/architecture-diagram.mmd">architecture-diagram.mmd</a> — rendered to brand-themed SVG/PNG (dark + light, plus 16:9 video cards) via <code>mermaid-cli</code>; see <a href="https://github.com/iarjunganesh/continuum/blob/main/assets/architecture/README.md">assets/architecture/README.md</a> for the regenerate command.</sub>
...
<p align="center">
  <a href="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/recovery-sequence-light.svg" target="_blank" rel="noopener noreferrer">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/recovery-sequence-dark.svg">
      <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/recovery-sequence-light.svg">
      <img width="820" src="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/recovery-sequence-light.svg"
...
<sub>Click to enlarge: <a href="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/recovery-sequence-light.svg" target="_blank" rel="noopener noreferrer">light</a> / <a href="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/architecture/recovery-sequence-dark.svg" target="_blank" rel="noopener noreferrer">dark</a> · Source: <a href="https://github.com/iarjunganesh/continuum/blob/main/assets/architecture/recovery-sequence.mmd">recovery-sequence.mmd</a></sub>
...
> **Deep dive** → **[`docs/ARCHITECTURE.md`](https://github.com/iarjunganesh/continuum/blob/main/docs/ARCHITECTURE.md)** — the dual memory model, the step-by-step recovery walkthrough, the vector-index DDL, and a typical-agent vs Continuum comparison.
...
| ADR | Decision |
| --- | --- |
| [001](https://github.com/iarjunganesh/continuum/blob/main/docs/adr/001-dual-memory-model.md) | Dual transactional + vector memory in one CockroachDB store — no separate vector DB to drift |
| [002](https://github.com/iarjunganesh/continuum/blob/main/docs/adr/002-stateless-lambda-recovery.md) | Stateless Lambda, no provisioned concurrency — every invocation must recover cold |
| [003](https://github.com/iarjunganesh/continuum/blob/main/docs/adr/003-mcp-readonly-queries.md) | MCP Server in read-only mode as the live query interface |
| [004](https://github.com/iarjunganesh/continuum/blob/main/docs/adr/004-ccloud-cli-audit-role.md) | ccloud CLI evaluated, then cut — 2 tools done well beats 3 done thin |
| [005](https://github.com/iarjunganesh/continuum/blob/main/docs/adr/005-synthetic-incident-data.md) | Synthetic incident corpus only — no real infra, ever |
| [006](https://github.com/iarjunganesh/continuum/blob/main/docs/adr/006-scope-cuts.md) | Explicit scope cuts, documented instead of hidden |
...
- **Distributed Vector Indexing** — `incident_embeddings.embedding VECTOR(1024)` with a C-SPANN index prefixed by `service`, so ANN search partitions per-service. The Correlation Agent's live query filters by structured columns *and* ranks by `<->` distance in one round trip. See [`infra/schema.sql`](https://github.com/iarjunganesh/continuum/blob/main/infra/schema.sql).
...
- **AWS SAM** — infrastructure as code ([`infra/template.yaml`](https://github.com/iarjunganesh/continuum/blob/main/infra/template.yaml)); the *absence* of `ProvisionedConcurrencyConfig` is a reviewable artifact rather than a console setting
...
Judging-criteria mapping and full submission narrative: [`submission/DEVPOST.md`](https://github.com/iarjunganesh/continuum/blob/main/submission/DEVPOST.md)
...
| **Correlation** | [![Vector Index](https://img.shields.io/badge/Vector_Index-C--SPANN_1024d-6933FF?logo=cockroachlabs&logoColor=white)](https://www.cockroachlabs.com/docs/stable/vector-indexes) | `service`-prefixed C-SPANN index; structured filter + `<->` ANN ranking in one round trip ([`infra/schema.sql`](https://github.com/iarjunganesh/continuum/blob/main/infra/schema.sql)) |
...
| **Agent Pattern** | [![Agent Pattern](https://img.shields.io/badge/5_agents-1_write_path-2563EB?logo=python&logoColor=white)](https://github.com/iarjunganesh/continuum/blob/main/agents/orchestrator.py) | Orchestrator · Correlation · Remediation · Memory · Query. `memory_agent.py` is the *only* module permitted to write state |
...
| **Compute** | [![AWS Lambda](https://img.shields.io/badge/AWS_Lambda-python3.14-FF9900?logo=awslambda&logoColor=white)](https://aws.amazon.com/lambda/) | Stateless orchestrator, deliberately **no provisioned concurrency** — every invocation proves cold recovery (ADR 002, [`infra/template.yaml`](https://github.com/iarjunganesh/continuum/blob/main/infra/template.yaml)) |
...
| **Orchestrator** | Live on AWS Lambda — `continuum-orchestrator`, eu-central-1 (stack `continuum`), deployed from CI on a version tag ([ADR 010](https://github.com/iarjunganesh/continuum/blob/main/docs/adr/010-deploy-on-tag-from-ci.md)). No provisioned concurrency: **cold start 1806 ms init, 130 MB / 512 MB** (sampled 2026-08-08 on the `v0.9.5` build; every tag since has changed no application code and sits inside the same 1578–1806 ms spread). Warm environments *are* reused between back-to-back invocations, and it doesn't matter — the orchestrator re-reads CockroachDB first either way, so the guarantee never rests on the container being new. [`docs/DEPLOY.md`](https://github.com/iarjunganesh/continuum/blob/main/docs/DEPLOY.md) is the authority on what is currently live |
| **Demo Video** | [https://youtu.be/LwD8__sKqa0](https://youtu.be/LwD8__sKqa0) — 2:55, 1920×1080/30, captions included. The kill and the resume are one continuous take, no cut between them. Script: [`submission/DEMO_SCRIPT.md`](https://github.com/iarjunganesh/continuum/blob/main/submission/DEMO_SCRIPT.md) |
| **Try It Now** | `make chaos-demo` — kill the agent mid-incident, watch it resume from CockroachDB |

Submission checklist: [`submission/SUBMISSION.md`](https://github.com/iarjunganesh/continuum/blob/main/submission/SUBMISSION.md) · Judging alignment + project story: [`submission/DEVPOST.md`](https://github.com/iarjunganesh/continuum/blob/main/submission/DEVPOST.md) · Cost model: [`submission/COSTS.md`](https://github.com/iarjunganesh/continuum/blob/main/submission/COSTS.md)

### Run it yourself — the interactive notebook

[![Open in Jupyter](https://img.shields.io/badge/Demo-Jupyter%20Notebook-F37626?logo=jupyter&logoColor=white)](https://github.com/iarjunganesh/continuum/blob/main/notebooks/DEMO_RUNBOOK.ipynb)

**[`notebooks/DEMO_RUNBOOK.ipynb`](https://github.com/iarjunganesh/continuum/blob/main/notebooks/DEMO_RUNBOOK.ipynb)** — a self-contained walkthrough of the kill-and-recover sequence. The recovery guarantee is easy to assert and hard to believe without watching it, so step through it yourself rather than taking the README's word.
...
Setup notes and conventions: [`notebooks/README.md`](https://github.com/iarjunganesh/continuum/blob/main/notebooks/README.md).

---

## Captured Evidence

Judge-facing evidence — real runs with raw CockroachDB snapshots, structured logs, and provenance manifests recording the exact commit, cluster and models used. Index: [`assets/README.md`](https://github.com/iarjunganesh/continuum/blob/main/assets/README.md).

**A real kill-and-recover run is captured and committed**: [`assets/chaos-run/local-a2bb201d/`](https://github.com/iarjunganesh/continuum/tree/main/assets/chaos-run/local-a2bb201d/) — a live orchestrator hard-killed mid-step (real `SIGKILL`, pid 38600, no graceful shutdown), read back out of the database at three phases *and* photographed in the providers' own consoles at each one:
...
Captured by `make chaos-capture`, which performs the kill *and* records it, and marks the folder `FAIL` rather than emitting evidence if the kill misses the execution window or a step runs twice. `correlation_source` and `reasoning_source` both read `bedrock`, so the live AWS path is provable from the rows rather than assumed. The `02-frozen` row is the one that cannot be staged after the fact — once a run resolves the console reads `resolved` — which is why `--pause` exists and why that frame is in [`local-a2bb201d/screenshots/`](https://github.com/iarjunganesh/continuum/tree/main/assets/chaos-run/local-a2bb201d/screenshots/).

**And the same three phases on the deployed function, with AWS delivering the kill**: [`assets/chaos-run/lambda-0b99a950/`](https://github.com/iarjunganesh/continuum/tree/main/assets/chaos-run/lambda-0b99a950/). `make chaos-capture-lambda` lowers the function's own timeout below its step-execution window, so **Lambda terminates the invocation mid-step** — no signal the runtime can catch, no cleanup, no checkpoint — and the resume is a second cold invocation of the same function. Step 0 froze `executing` with nothing alive to own it, resumed at that exact index, and the incident resolved with 3 steps executed and 0 duplicated, every durable step recording `runtime: lambda` from the function's own environment. The folder carries the function's **own CloudWatch log** for the window, which reads in order: `INIT_START` on `python:3.14` → `step_checkpoint_start` → `REPORT … Status: timeout` → a *second* `INIT_START` → `recovered_incident_state` with `last_step_status: executing`. That is the entire thesis, written by AWS rather than by this project.
...
| `make chaos-capture-lambda` | `python scripts/chaos_capture.py --via-lambda --profile continuum-admin` |
| `make clean-clone-check` | `python scripts/clean_clone_check.py` |
...
`.\scripts\chaos_demo.ps1` and the steps in [`docs/DEPLOY.md`](https://github.com/iarjunganesh/continuum/blob/main/docs/DEPLOY.md).
...
Beyond tests: structlog JSON logging across every agent, secrets via environment only, least-privilege IAM, and documented scope cuts (ADR 006) rather than hidden ones. Security posture and known limitations: [`SECURITY.md`](https://github.com/iarjunganesh/continuum/blob/main/SECURITY.md). Cost model and guardrails: [`submission/COSTS.md`](https://github.com/iarjunganesh/continuum/blob/main/submission/COSTS.md).
...
<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/demo-cards/signoff-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/demo-cards/signoff-light.svg">
    <img width="560" src="https://raw.githubusercontent.com/iarjunganesh/continuum/main/assets/demo-cards/signoff-light.svg"
