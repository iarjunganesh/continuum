...
## Decision
If ccloud CLI is included at all, its only job is: before the chaos-kill demo runs, verify that the CockroachDB cluster's backups and cross-region replication are healthy (`ccloud cluster backup list`, `ccloud cluster describe` style checks; the exact commands were never pinned against the current ccloud CLI docs, because the option was cut before build — see the resolution below). This ties directly into the project's narrative — a memory system that can't verify its own durability isn't trustworthy — rather than being a standalone integration.
