...
- **Qualified by ADR 008**: `bedrock-runtime` calls target a separate `BEDROCK_REGION` setting rather than `AWS_REGION`, so Bedrock can be moved without redeploying the Lambda. That seam was introduced because an account-level quota clamp held every region at an effective `0` and only eu-north-1 ever accepted calls; the clamp lifted 2026-08-06, and **as of ADR 008 addendum 3 (2026-08-01) `BEDROCK_REGION` defaults back to eu-central-1**. So the co-location argued for here now holds for all three legs — Lambda, CockroachDB and Bedrock — with the separate setting retained purely as an escape hatch for dynamic quotas.
