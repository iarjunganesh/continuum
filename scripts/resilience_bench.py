...
# Paths whose contents could change what these suites measure. Everything else
# being uncommitted is bookkeeping; something here being uncommitted means the
# numbers may describe code that exists on no commit at all.
MEASURED_PATHS = ("agents/", "api/", "observability/", "config.py", "scripts/resilience_bench.py", "infra/schema.sql")


def _code_provenance() -> dict:
    """What commit these numbers describe, and whether that is the whole truth.
    ...
    `git status --porcelain` alone is the wrong signal, and reporting it was
    actively misleading. It counts untracked files — and this script's own
    output directory and rewritten report are untracked or modified *by
    definition* while it runs, so every run self-reported "working tree dirty".
    A reader then cannot distinguish "the report file was being written" from
    "the orchestrator had uncommitted changes", and the previous run needed a
    hand-written paragraph in this generated file to explain which it was —
    a paragraph the next regeneration destroyed.

    So separate the two: modified *tracked* files under MEASURED_PATHS are the
    only ones that can invalidate a number. Everything else is noted, not
    flagged.
    """
    ...
    try:
        ...
        # `diff --name-only HEAD` rather than parsing porcelain status codes:
        # it yields bare paths, and it is tracked-only by construction, so
        # untracked output (this script's own run directory) cannot leak in.
        tracked = [ln.strip() for ln in git("diff", "--name-only", "HEAD").splitlines() if ln.strip()]
        measured = sorted(f for f in tracked if any(f.startswith(p) for p in MEASURED_PATHS))
        return {"sha": sha or "(unknown)", "dirty_tracked": sorted(tracked), "measured_dirty": measured}
...
def _provenance_note() -> str:
    """The staleness audit, generated rather than hand-written.

    It used to be a hand-maintained section in this file. That could not
    survive: this file is regenerated by `make resilience-bench`, so every run
    ...
    if p.get("from_manifest") and not p.get("granular"):
        return (
            ...
            f"predates per-file provenance, so it records only a single "
            f"{'dirty' if p.get('dirty_flag') else 'clean'} flag — not which files. "
            + (
                "That flag counted **untracked** files too, and a run's own evidence folder is "
                "untracked while it is being written, so every run of that vintage reported dirty; "
                "it is not evidence that measured code was uncommitted. "
                if p.get("dirty_flag")
                else ""
            )
            + "The numbers below are the ones it captured; the commit is the one it captured them at."
        )
    if p["measured_dirty"]:
        files = ", ".join(f"`{f}`" for f in p["measured_dirty"])
        return (
            f"> **These numbers may not describe any commit.** {files} had uncommitted changes when "
            f"this ran, and those paths can change what is measured. Re-run from a clean tree before "
            f"citing anything here."
        )
    others = [f for f in p["dirty_tracked"]]
    aside = ""
    if others:
        aside = (
            f" ({len(others)} other tracked file(s) were modified — "
            f"{', '.join(f'`{f}`' for f in others[:3])}{' …' if len(others) > 3 else ''} — "
            f"none of which affect what is measured; this report is normally one of them, "
            f"since the run rewrites it.)"
        )
    return (
        f"> **No measured code was uncommitted when this ran.** Nothing under "
        f"`agents/`, `api/`, `observability/`, `config.py`, `infra/schema.sql` or this script "
        f"differed from `{p['sha']}`, so every table below describes exactly that commit.{aside}\n>\n"
        f"> If `{p['sha']}` is now an ancestor of a `HEAD` that changed any of those paths, treat "
        f"these numbers as stale and re-run — that is the failure this note exists to make visible."
    )


def _suite_d() -> str:
    """Suite D, rendered from the drill's own committed evidence.

    Suite D is measured by `scripts/deploy_restart_drill.py`, not by this
    script — but it belongs in this report, and it was originally written here
    by hand. That could not survive: this file is regenerated by
    `make resilience-bench`, so the run on 2026-08-07 silently deleted the whole
    section. Reading it back out of `assets/deploy-restart-run/` means the
    newest drill is always the one described, and a bench run cannot destroy it.

    When no drill evidence exists the section still renders, saying so — an
    explicit gap is recoverable, a silently missing suite is not.
    """
    runs = []
    for man_path in sorted(Path("assets/deploy-restart-run").glob("*/manifest.json")):
        try:
            man = json.loads(man_path.read_text(encoding="utf-8"))
            hits = sorted((man_path.parent / "evidence").glob("*_deploy-restart.json"))
            if hits:
                runs.append((man.get("started_utc", ""), man, json.loads(hits[0].read_text(encoding="utf-8"))))
        except Exception:  # noqa: BLE001 — a malformed run must not fail the report
            continue

    header = "## D. Deploy mid-incident — the code replaced underneath an open step\n"
    if not runs:
        return (
            f"{header}\n"
            "> **Not yet measured.** No run exists under `assets/deploy-restart-run/`. "
            "Produce one with `make deploy-restart-drill CLONE_DIR=<clean checkout>`.\n"
        )

    started, man, d = max(runs, key=lambda r: r[0])
    run_id = man.get("run_id", "(unknown)")
    sha = man.get("code", {}).get("commit_short", "(unknown)")
    when = dt.datetime.fromisoformat(started).astimezone(dt.timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    before, after = d["before"]["code_sha256"], d["after"]["code_sha256"]
    rec = d.get("recovery_payload", {})
    corr, reas = rec.get("correlation_source"), rec.get("reasoning_source")
    sources = f"`{corr}`" if corr == reas else f"`{corr}` and `{reas}`"
...
**Run:** {when} · run id `{run_id}` · code `{sha}` · `make deploy-restart-drill`

| Phase | Observed |
| --- | --- |
| Interrupt | AWS timed the invocation out (`Timeout` {d["interrupt"]["timeout_used"]} s, restored to \
{d["interrupt"]["timeout_restored"]} s in a `finally`) |
| Durable after kill | step {d["durable_after_kill"]["step_index"]}, \
`{d["durable_after_kill"]["status"]}`, {d["durable_after_kill"]["steps"]} row |
| Deploy | {d["deploy"]["seconds"]:.1f} s · `CodeSha256` `{before[:8]}…` → `{after[:8]}…` · revision id changed |
| Cold resume | **same incident**, step {rec.get("step_index")} re-executed on the new build in \
{d["resume_ms"]:.0f} ms |
| Duplication | **none** — {d["rows_for_step"]} row for the step |

The resumed invocation returned `resumed: {str(rec.get("resumed")).lower()}`,
`reexecuted_after_interrupt: {str(rec.get("reexecuted_after_interrupt")).lower()}`, and
`correlation_source` / `reasoning_source` both {sources} — so the new build ran the live AWS path,
...
`code_replaced: {str(d["code_replaced"]).lower()}` and `revision_changed: \
{str(d["revision_changed"]).lower()}` are asserted, not narrated.
...
[`assets/deploy-restart-run/{run_id}/`](../assets/deploy-restart-run/{run_id}/).
...
"""
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(doc)
    # Deliberately not echoing the document. It is written as UTF-8, but the dev
    # machine's console is cp1252 and dies on the first `≈` — after the file is
    # already on disk, so the run looks failed while having fully succeeded.
...
def main() -> None:
    ...
    p.add_argument(
        "--render-from",
        metavar="RUN_DIR",
        help="re-render --out from a committed assets/resilience-run/<id>/ instead of measuring. "
        "Touches no cluster and takes no new samples — for prose or provenance changes to a "
        "generated report whose numbers are already evidence",
    )
    p.add_argument(
        "--no-evidence",
        action="store_true",
    ...
    # Re-render the report from a committed run without re-measuring anything.
    #
    # The report is a generated file, so a change to its prose or provenance
    # block could previously only be applied by re-running every suite — half an
    # hour against a live cluster, hundreds of real incidents written and
    # deleted, purely to restate numbers that were already saved as JSON beside
    # it. That cost is what makes people hand-edit generated files instead.
    if args.render_from:
        run_dir = Path(args.render_from)
        ev = run_dir / "evidence"
        if not ev.is_dir():
            p.error(f"{run_dir} has no evidence/ directory")

        def load(suffix: str, default=None):
        ...
        _write(
            ...
            load("lambda-timeout"),
            load("exactly-once", []),
            load("agent-throughput", []),
            load("vector-scale", []),
        )
        print(f"re-rendered {args.out} from {run_dir} — no cluster access, no new measurements")
        return
