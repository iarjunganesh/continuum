...
**The judge-facing evidence is complete as of 2026-08-09.** Both chaos runs are captured *and* screenshotted — `assets/chaos-run/local-a2bb201d/` (a real `SIGKILL`) and `assets/chaos-run/lambda-0b99a950/` (AWS delivering the kill via a lowered function timeout), seven console frames each, shot inside the `--pause` window that is the only moment the interrupted state exists. `make clean-clone-check` answers the "runs from a clean clone" item with a report that states its own limits. **The demo video is shot, cut and published (2026-08-12): https://youtu.be/LwD8__sKqa0** — 2:55.7, beats 6–8 one take with no splice between the kill and the resume, captions from `assets/demo-video/continuum.srt`. **Visibility is confirmed Public** (2026-08-13): the watch page's own embedded player metadata reports `"isUnlisted":false` and `"isPrivate":false` to an unauthenticated fetch, which settles from outside YouTube Studio what a plain 200 cannot — Public and Unlisted both serve the page to a signed-out visitor. **The Devpost form answers are pasted and cross-verified** against the live project page the same day. Remaining: one judge-experience dry run from someone who has not seen the project, and the two honestly-open boxes in `submission/SUBMISSION.md` (the optional CockroachDB-tools feedback, and watching the video start to finish).
...
make precision-check    # recompute precision@1 from the committed fixture — offline, no AWS
...
- **The Cloud cluster serves the demo and nothing else.** `make resilience-bench`, `make benchmark`,
  ...
  5.82M against a 50M/month allowance as of 2026-08-10, and that already includes the heaviest day the project has run); it is that an N=200 bench left **665 incidents, 431 frozen in
...
- `tests/unit/test_precision_at_1.py` — the exception to "one file per agent": it tests a *claim*, not a module. `precision@1 55% → 98%` is the evidence that the vector index retrieves precedent that means something, and it was measured once by hand and quoted in five documents before anything recomputed it — the same trusted-and-unchecked shape as the bug it describes. `scripts/precision_check.py` (`make precision-check`) re-derives both arms from the committed fixture with no AWS call and no cluster; the tests fail if the Titan arm drops below 90% or the deliberately-meaningless baseline catches up. Keep the failure-mode taxonomy *imported* from `generate_synthetic_incidents.py` rather than restated, or the check silently stops covering part of the corpus.
...
- Judge-facing evidence: `assets/` — `assets/README.md` is the curated index; `assets/chaos-run/` is captured and screenshotted (both environments, 2026-08-09). **Never hand-edit a frame in `assets/provider-evidence/` or any `screenshots/` folder.** Masking is declared as data in `scripts/redact_evidence.py` with a reason per region, is idempotent, and `--check` fails on an unmasked declared region **or on a screenshot with no declaration at all** — an empty region tuple is a valid declaration and means "someone looked and it needs nothing". It covers `provider-evidence/`, `chaos-run/*/screenshots/`, `demo-video/statics/` **and the video files themselves** (`demo-video/*.mp4`, checked by sampling frames) — a still frame cut into the video is as judge-facing as anything else, and the video is more so. That last family was added on 2026-08-12 after `mcp-query-take.mp4` shipped with the signed-in user's photograph in every one of its 315 frames: the gate walked PNGs, so nothing complained. Note the avatar sits **three pixels right** in an OBS recording than in a DevTools capture (`AVATAR_TAKE`, not `AVATAR`) — reusing the box leaves the documented crescent. Video checks are tolerant of a few units per channel because yuv420p does not preserve an exact RGB value; an exact-equality test reports a correctly-masked file as unmasked and re-encodes it on every run. It may cover a real person's photograph and an AWS account id, and nothing else — masking a metric, timestamp or identifier that a document cites would turn evidence into decoration. A region's coordinates are *measured off the capture*, never eyeballed: an eyeballed box once left a crescent of the photograph showing, and the account tooltip sits ~16px further right on CloudWatch's log-events page than on its metrics pages, which is why two boxes exist
