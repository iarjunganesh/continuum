...
- Bedrock calls target a **separate** `BEDROCK_REGION` (default `eu-north-1`), not `AWS_REGION` — this account's Bedrock quotas are dynamically adjusted and probe as effectively `0` in most regions on most days, for first-party (Titan, Nova) and Anthropic models alike (ADR 008 + addendum). The split is intentional, not the drift the line above warns about; don't "fix" it back to `AWS_REGION`. Run `make probe-bedrock` before a demo — both Bedrock paths degrade silently by design (no precedent / precedent-replay), so a throttled account *looks* fine while never touching Bedrock.
