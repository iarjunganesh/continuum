...
**The Bedrock account quota clamp (ADR 008) was LIFTED on 2026-08-06** via an AWS Support eligibility review — `make probe-bedrock` now returns OK for every candidate region × both models. No config change was needed; the `BEDROCK_REGION` split stands as documented. Two consequences: (1) quotas remain *dynamic*, so re-probe immediately before recording rather than trusting a days-old green run; (2) **the live Bedrock path has never actually executed** in this project's history — correlation and remediation ran on silent fallbacks throughout, so the real Titan/Claude response-handling code is unproven rather than proven-good. Exercise both paths end to end before relying on them.

Remaining before submission: verify the live Bedrock paths, run the orchestrator deploy to Lambda, capture the judge-facing evidence runs into `assets/chaos-run/` (see `assets/README.md`), record the demo video (`submission/DEMO_SCRIPT.md`), and fill in the `submission/SUBMISSION.md` checklist end to end.
...
- Bedrock calls target a **separate** `BEDROCK_REGION` (default `eu-north-1`), not `AWS_REGION` (ADR 008). The split is intentional, not the drift the line above warns about; don't "fix" it back to `AWS_REGION`. It originated with an account-level quota clamp that probed as effectively `0` across every region and model — **that clamp was lifted 2026-08-06**, but quotas remain dynamic, so the split stays and `make probe-bedrock` stays a pre-demo step. Both Bedrock paths degrade silently by design (no precedent / precedent-replay), so a throttled account *looks* fine while never touching Bedrock — the probe is the only way to know which mode you're in.
